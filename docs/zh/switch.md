...
一般不同的业务流程, 可能都不仅仅只涉及单个接口, 整个流程会有到多个接口的请求

而流程中根据接口返回的数据不同, 又会产生多条分支, 这个时候涉及到多个接口多个状态的切换, 可能会比较麻烦

这个时候可以在 mock 文件夹下新增`_set`目录, 用于存放多个接口的 mock 数据
...
如果某个接口, 在前端 mock 面板已经勾选了响应数据, 同样在 set 面板中勾选的也包含同一个接口
...
当勾选的 set 配置中没有匹配到对应的接口, 才会再次尝试匹配单独定义的接口, 如果命中再返回 mock 面板中勾选的数据
